...
namespace
{
    inline wchar_t GetSingleHexDigit(uint32_t value)
    {
        value &= 0xF;
...
TextTree::NodePointer TextTree::NodePointer::operator[](__in_z const wchar_t* keyName) const
{
    return Find(keyName, static_cast<uint32_t>(wcslen(keyName)), /*shouldSearchImmediateChildren*/ true);
}
...
TextTree::NodePointer TextTree::NodePointer::end()
{
    ...
    ||  !textTree_.AdvanceNode(AdvanceNodeDirectionSiblingNextEnd, 1, IN OUT pointer.nodeIndex_))
    {
...
std::wstring TextTree::NodePointer::GetText() const
{
    std::wstring text;
...
std::wstring TextTree::NodePointer::GetSubvalue() const
{
    std::wstring text;
...
std::wstring TextTree::NodePointer::GetSubvalue(const std::wstring& defaultText) const
{
...
std::wstring TextTree::NodePointer::GetSubvalue(__in_ecount(defaultTextLength) const wchar_t* defaultText, uint32_t defaultTextLength) const
{
    std::wstring text;
...
void TextTree::GetText(uint32_t nodeIndex, OUT std::wstring& text) const
{
...
void TextTree::SetText(__inout Node& node, const std::wstring& text)
{
    SetText(node, text.c_str(), static_cast<uint32_t>(text.size()));
}


void TextTree::SetText(__inout Node& node, __in_ecount(textLength) const wchar_t* text, uint32_t textLength)
{
    assert(size_t(&node - nodes_.data()) < nodes_.size());
    ...
    ) const
{
    ...
    while (nodeIndex < nodesCount)
    {
        ...
        if (currentTextLength == textLength && _wcsnicmp(text, currentText, currentTextLength) == 0)
        {
    ...
    ) const
{
    return Find(
                ...
                keyName,
                static_cast<uint32_t>(wcslen(keyName)),
...
bool TextTree::SetSubvalue(
    uint32_t keyNodeIndex,
    __in_ecount(valueTextLength) const wchar_t* valueText,
...
void TextTree::Append(TextTree::Node::Type type, uint32_t level, __in_ecount(textLength) wchar_t const* text, uint32_t textLength)
{
    TextTree::Node node = {};
    node.start = static_cast<uint32_t>(nodesText_.size());
    node.length = textLength;
    ...
    )
{
    ...
    if (ch == '/') // Start of comment.
    {
        ...
        if (ch != '/')
        {
            ReportError(textIndex_, L"Expect two forward slashes for a comment or for slashes to be used for named closure or in quoted strings.");
    ...
    else if (ch == '"') // Word is quoted.
    {
        ...
        while (textIndex_ < textLength_)
        {
            ...
            if (JsonexIsControlCharacter(ch)) // Control character found inside string which was not escaped.
            {
                ReportError(textIndex_, L"Control characters found inside string. They must be escaped.");
                SkipInvalidWordCharacters(); // Skip the invalid word.
                return false;
            }
    ...
    else // Unquoted identifier or value.
    {
        while (textIndex_ < textLength_)
        {
            ...
            if (!JsonexIsValidWordCharacter(ch))
            {
                ...
                ReportError(textIndex_, L"Unsupported characters found inside identifier. Put the identifier in quotes.");
                SkipInvalidWordCharacters(); // Skip the invalid word.
                return false;
            }
    ...
    )
{
    for (;;)
    {
        ...
        case '=':
            if (!haveWord)
            {
                ReportError(textIndex_ - 1, L"Assignment must have a keyword identifier before it.");
                break;
            }
    ...
    )
{
    ...
    else if (firstCh == '"') // Word is quoted.
    {
        ...
        while (textIndex_ < textLength_)
        {
            ...
            if (IniIsNewLineCharacter(ch))
            {
                ReportError(textIndex_, L"Quoted word ends before closing quote '\"'.");
                break;
            }
    ...
    )
{
    ...
    case '[':
        ...
        {
            ReportError(textIndex_, L"Section name is missing closing bracket ']'.");
        }
        SkipSpacesAndLineBreaks();
        break;
...
const wchar_t* TextTreeWriter::GetText(__out uint32_t& textLength) const throw()
{
...
void TextTreeWriter::GetText(OUT std::wstring& text) const
{
    text = text_;
}
...
uint32_t TextTreeWriter::GetTextLength(
    __in_ecount_opt(textLength) const wchar_t* text,
    uint32_t textLength
    ) throw()
{
    if (text == nullptr)
    ...
    return static_cast<uint32_t>(wcslen(text));
}
    ...
    )
{
    ...
    wchar_t const* p = text;
    wchar_t const* pEnd = text + textLength;
    ...
    )
{
    ...
    {
        const wchar_t newLineCharacters[2] = {'\r', '\n'};
...
HRESULT RunTests()
{
    const wchar_t* testString = L"thistest=foo bar(stuff:boo cat[1 2]) singleitem singleitem2";

    // Read all nodes into a text tree.
    TextTree nodes;
    uint32_t testStringLength = static_cast<uint32_t>(wcslen(testString));
    ...
    auto i = nodes.begin();
    auto s = i[L"bar"][L"stuff"].GetSubvalue();
    auto bar = i[L"bar"];
    auto b = bar.begin();
    auto e = bar.end();
    auto zar = i[L"zar"];
    auto b2 = zar.begin();
    auto e2 = zar.end();
    auto sing = i[L"singleitem"];
    auto b3 = sing.begin();
    auto e3 = sing.end();
    auto sing2 = i[L"singleitem2"];
    auto b4 = sing2.begin();
    auto e4 = sing2.end();
    auto j = i;
    ++j;
    ...
    auto s4 = i.GetSubvalue(L"Hello", 5);

    MessageBox(nullptr, t.c_str(), L"Caption", MB_OK);
