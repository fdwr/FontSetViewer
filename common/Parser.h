...
public:
    ...
    struct Node
    {
        ...
        inline void GetText(const TextTree& textTree, OUT std::wstring& text) const { textTree.GetText(*this, OUT text); };
    };
    ...
    public:
        ...
        void MarkInvalid();
        NodePointer operator[](__in_z const wchar_t* keyName) const;
        std::wstring GetText() const; // Returns empty string if it does not exist.
        std::wstring GetSubvalue() const; // Returns empty string if it does not exist.
        std::wstring GetSubvalue(const std::wstring& defaultText) const; // Supply a default value if the value is not found.
        std::wstring GetSubvalue(__in_ecount(defaultTextLength) const wchar_t* defaultText, uint32_t defaultTextLength) const;
        NodePointer begin();
        NodePointer end();
...
public:
    ...
    void GetText(uint32_t nodeIndex, OUT std::wstring& text) const;

    // Set the text.
    // The node must be one from this tree, retrieved via GetNode.
    void SetText(__inout Node& node, const std::wstring& text);

    void SetText(__inout Node& node, __in_ecount(textLength) const wchar_t* text, uint32_t textLength);
    ...
    void Append(TextTree::Node::Type type, uint32_t level, __in_ecount(textLength) wchar_t const* text, uint32_t textLength);
...
public:
    ...
    struct Error
    {
        ...
        const wchar_t* errorMessage;    // Weak pointer to static text data.
    };
...
public:
    ...
    //    
    //      wstring, vector<wchar_t>, initializer_list<wchar_t>, wchar_t[n]
    //
...
protected:
    __field_ecount_opt(textLength_) wchar_t const* text_ = nullptr;   // Weak pointer should be valid for lifetime of the parsing operation (or until Reset).
...
public:
    ...
    const wchar_t* GetText(__out uint32_t& textLength) const throw();

    // Reads the text into the string.
    void GetText(OUT std::wstring& text) const;
...
public:
    ...
    HRESULT BeginArray(
        __in_ecount_opt(textLength) const wchar_t* text = nullptr,
        uint32_t textLength = 0xFFFFFFFF
        );
